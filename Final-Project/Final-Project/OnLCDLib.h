...
OnLCDLib - LCD Library written in C for AVR Microcontrollers v2.0
...
void LCDGotoXY(uint8_t x, uint8_t y);
	...
	void LCDWriteFloat(double float_number, int8_t nrOfDigits, uint8_t nrOfDecimals);
#endif
...
#if LCD_DISPLAY_FLOATS == TRUE
	void LCDWriteFloat(double float_number, int8_t nrOfDigits, uint8_t nrOfDecimals){
		int32_t integer_part_1 = float_number;
		int32_t integer_part_2 = 0;
		double fractional_part = float_number - integer_part_1;
		uint8_t float_length = nrOfDecimals;
		
		if(nrOfDecimals == 0) float_length = 1;
		...
		integer_part_2 = fractional_part;

		// Convert fractional part to a positive 
		if(integer_part_2 < 0) integer_part_2 = 0 - integer_part_2;

		// The case where the float is -0.xxxx
		if(float_number < 0 && integer_part_1 == 0){
			LCDWriteString("-");
		}

		LCDWriteInt(integer_part_1, nrOfDigits);
		LCDWriteString(".");
		LCDWriteInt(integer_part_2, nrOfDecimals);
	}
#endif
